...
export async function onCall({ message, args }) {
    try {
        ...
        let pathImg = join(global.cachePath, `__lexi_${senderID}_${Date.now()}.png`);
        let text = args.join(" ");

        if (!text) return message.reply("Enter the content of the comment on the board");

        await global.downloadFile(pathImg, "https://i.imgur.com/hTU9zhX.png");
        ...
        const lines = await wrapText(ctx, text, 490);
