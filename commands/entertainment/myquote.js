...
const config = {
    name: "myquote",
    aliases: ["mq"],
    version: "1.0.0-xaviabot-port-refactor",
...
async function onCall({ message, args, prefix }) {
    ...
    if (input.length !== 2) {
        message.reply(
            `To use this command, simply type “${prefix}${config.name} [text] = [author]”, where [text] is the quote you want to save and [author] is the name of the person who said it.` +
                '\nFor example, to save the quote "The best and most beautiful things in the world cannot be seen or even touched - they must be felt with the heart" by Helen Keller,' +
                `\nYou would type “${prefix}${config.name} The best and most beautiful things in the world cannot be seen or even touched - they must be felt with the heart = Helen Keller.”`
        );
        return;
    }

    const [quoteText, authorName] = input;
    const words = quoteText.split(" ");

    if (words.length > 25) {
        message.reply("The quote text must be 25 words or less only.");
        return;
    }

    const savePath = path.join(
        global.cachePath,
        `${Date.now()}_mq_${message.messageID}.png`
    );
    ...
    try {
        ...
        if (global.isExists(savePath)) unlinkSync(savePath);
    }
}
